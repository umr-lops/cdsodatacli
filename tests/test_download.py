...
def test_filter_product_already_present(mock_conf):
    ...
    with patch("cdsodatacli.download.check_safe_in_archive",
           side_effect=lambda safename, conf: archive_map.get(safename, (False, None))) as mock_archive, \
     patch("cdsodatacli.download.check_safe_in_spool",
...
def test_add_missing_cdse_hash_ids_in_listing(tmp_path):
    ...
    with patch("cdsodatacli.download.ExplodeSAFE", return_value=mock_exploded), \
...
def test_download_list_product_sequential(
    ...
    with patch("cdsodatacli.download.remove_semaphore_token_file"), \
        ...
        mock_filter.return_value = (pd.DataFrame({
            "safe": ["SAFE1"], "id": ["id1"], "outputpath": ["/tmp/SAFE1.zip"], "urls": ["url1"]
        }), defaultdict(int))
