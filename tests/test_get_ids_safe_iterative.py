...
from cdsodatacli.scripts.get_ids_listing_safe_iterative import add_ids_to_listing_iterative
...
def test_add_ids_to_listing_iterative_full_success(sample_listing, tmp_path):
    ...
    with patch("cdsodatacli.download.add_missing_cdse_hash_ids_in_listing", return_value=mock_df):
...
def test_add_ids_to_listing_iterative_multi_step(sample_listing, tmp_path):
    ...
    mock_response_1 = pd.DataFrame({
        "safename": ["S1A_IW_GRDH_1SDV_20220503T000000", "S1A_IW_GRDH_1SDV_20220503T000001"],
        "id": ["uuid0", "uuid1"]
    })
    ...
    with patch("cdsodatacli.download.add_missing_cdse_hash_ids_in_listing") as mocked_api:
...
def test_add_ids_to_listing_no_progress_break(sample_listing, tmp_path):
    ...
    with patch("cdsodatacli.download.add_missing_cdse_hash_ids_in_listing", return_value=empty_df):
        result_file = add_ids_to_listing_iterative(sample_listing, output_path)
...
def test_add_ids_to_listing_duplicates_in_api(sample_listing, tmp_path):
    ...
    mock_df = pd.DataFrame({
        "safename": ["S1A_IW_GRDH_1SDV_20220503T000000", "S1A_IW_GRDH_1SDV_20220503T000000"],
        "id": ["uuid0", "uuid0"]
    })
