...
def get_a_free_s3_session(active_s3_sessions_status,conf,account_group,blacklist=None):
    """
        ...
        for login in active_s3_sessions_status:
            if login not in blacklist:
                for session_id in active_s3_sessions_status[login]:
                    if active_s3_sessions_status[login][session_id] is False and candidate_session is None:
                        ...
                        credentials_from_conf_file = get_a_credentials_from_conf_file(conf=conf,account_group=account_group,login=login)
                        s3_long_term_credentials = {'s3-access-key':credentials_from_conf_file['s3-access-key'],
                                                    's3-secret': credentials_from_conf_file['s3-secret'],}
                        break
    return active_s3_sessions_status,candidate_session,candidate_login,s3_long_term_credentials
...
def get_sessions_download_available_s3(conf,active_s3_sessions_status, subset_to_treat, blacklist=None, logins_group="logins"):
    """
    ...
    for ss in range(len(subset_to_treat)):
        ...
        active_s3_sessions_status, candidate_session,\
            candidate_login,s3_credentials = get_a_free_s3_session(active_s3_sessions_status,
                                                        conf=conf,
                                                        account_group=logins_group,
                                                        blacklist=blacklist)

        if candidate_session is None:
            logging.debug("no more S3 session available for now in that group of logins.")
            break  # no more account free
        else:
    ...
    df_products_ready_for_download["S3Path"] = bunch_s3path_to_download  # to check S3path
