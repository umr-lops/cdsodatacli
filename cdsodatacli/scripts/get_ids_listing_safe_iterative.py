...
def add_ids_to_listing_iterative(input_listing, output_listing=None):
    ...
    tmplisting = input_listing
    loop_cpt = 0
    logging.getLogger().setLevel(logging.CRITICAL) # Attention, cela coupe les logs globaux
    ...
    while cpt_ids_found != len(df_target['id']) and loop_cpt < 20: 
        ...
        new_cpt = (df_target['id'].isna() == False).sum()
        if new_cpt == cpt_ids_found and loop_cpt > 1:
            logging.warning("Aucun nouvel ID trouvé lors de cette itération, arrêt de la boucle.")
            break
        ...
        tmplisting = os.path.join(tempfile.gettempdir(), 'tmp_missing_id_listing_cdsodatacli.txt')
        dfmissing['safename'].to_csv(tmplisting, header=False, index=False)
        ...
        tmplisting_safeid = os.path.join(tempfile.gettempdir(),
                                         'tmp_safe_id_listing_cdsodatacli_probably_incomplet.txt')
        df_target[['id','safename']].to_csv(tmplisting_safeid, header=False, index=False)
    ...
    if len(df_target) != len(df):
        logging.warning(f"Attention: Output lines ({len(df_target)}) != Input lines ({len(df)})")
    # -------------------------------------------------
